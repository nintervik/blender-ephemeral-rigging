...
class EphemeralRigSystem:
    ...
    def on_transform_selected(self, transform: str):
        ...
        # TODO: Key pose of current selected rig and update saved pose data in scene or JSON. If "auto-key" is ON, otherwise
        # the user need to key the pose manually
...
class EphemeralRig:
    ...
    def generate_transform_uids(transforms: List[str]) -> dict:
        """
        """
        
        for tr in transforms:
            # TODO: Store as a metadata attr in the transform
            self.transforms[uuid.uuid4()] = transform
